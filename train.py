...
dataset_loader = image_processing.DatasetLoader(image_paths, (1350, 128, 128, 3), verbose=1)
...
model = KNeuralNet.build(batch=1215, width=128, height=128, depth=3, classes=3)
model.compile(optimizer=optimizer, loss="categorical_crossentropy", metrics=["accuracy"])

print("[INFO] training network ....")
train = model.fit(trainX, trainY, validation_data=(testX, testY), batch_size=16, epochs=15, verbose=1)
...
print(classification_report(testY.argmax(axis=1), predictions.argmax(axis=1), target_names=["1_kor", "2_khor", "3_kur"]))
